...
def _fedavg(clientid2arch, n_selected_clients, flatten_local_models, client_models,weights = None):
    if weights == None:
        weights = [
            torch.FloatTensor([1.0 / n_selected_clients]).cuda() for _ in range(n_selected_clients)
        ]
    ...
    for client_idx, flatten_local_model in flatten_local_models.items():
        ...
        local_models[client_idx] = _model.cuda()
...
def fedavg(
    ...
    if (
        ...
        and "weighted" not in  conf.fl_aggregate["server_teaching_scheme"])
    ):
        # directly averaging.
        ...
        for idx, (client_id, local_model) in enumerate(local_models.items()):
            ...
            # check the perf.
            if "drop" in conf.fl_aggregate["server_teaching_scheme"]:
                if perf < random_guess_perf:
                    indices_to_remove.append(idx)

        if 'weighted' in conf.fl_aggregate["server_teaching_scheme"]:

...
class Scheduler(object):
    ...
    def init_learning_rate(self):
        ...
        # if warmup.
        if self.conf.lr_warmup_epochs is None and self.conf.lr_warmup:
    ...
    def init_lr_scheduler(self):
        if self.conf.lr_scheduler == "MultiStepLR":
            ...
            scheduler_info = (
                f"use ReduceLROnPlateau scheduler"
            )
        elif self.conf.lr_scheduler == "WarmupCosine":
            lr_scheduler = WarmupCosineSchedule(
                self.optimizer,
                warmup_steps=self.conf.warmup_rounds,
                t_total=self.conf.n_comm_rounds
            )
            scheduler_info = (
                f"use WarmupCosine scheduler"
            )
        else:
            raise NotImplementedError(

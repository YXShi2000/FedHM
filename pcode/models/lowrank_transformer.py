...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., apply_lora=False, lora_r=0,
                 lora_alpha=0):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                              apply_lora=apply_lora, lora_r=lora_r, lora_alpha=lora_alpha)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, apply_lora=False, lora_r=0,
            lora_alpha=0):
        """
        Args:
...
def vit(conf, arch=None):
    factor = 1

    if arch is not None and len(arch.split('_')) > 1:
...
# reload_state_dict = copy.deepcopy(model.state_dict())
# for param_name, param in model.state_dict().items():
#     if re.match(attn_weight_pattern, param_name) is None:
#         continue
#     prefix_name = param_name.rsplit('.', 1)[0]
#
#     U, VT = spectral_init(param, 4)
#     reload_state_dict[prefix_name + '.lora_A'].data[:VT.shape[0], :] = VT
#     reload_state_dict[prefix_name + '.lora_B'].data[:, :U.shape[1]] = U
